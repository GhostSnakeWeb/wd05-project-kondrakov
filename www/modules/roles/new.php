<?php 

$title = "Создать новую роль";

if (isset($_POST['roleNew'])) {

	if (trim($_POST['roleName']) == '') {
		$errors[] = ['title' => 'Введите название роли'];
	}

	if (empty($errors)) {
		$role = R::dispense('roles');
		$role->role_name = htmlentities($_POST['roleName']);
		R::store($role);
		header("Location: " . HOST . "roles?result=roleCreated");
		exit();
	}

}

//Готовим контент для центральной части
//ob_start() - буферизированный вывод.
ob_start();
include ROOT . "templates/_parts/_header.tpl";
include ROOT . "templates/roles/new.tpl";
//ob_get_contents() - получает контент записанный между функциями ob_start и ob_get_contans из буфера.
$content = ob_get_contents();
ob_end_clean();

//Выводим шаблоны
include ROOT . "templates/_parts/_head.tpl";
include ROOT . "templates/template.tpl";
include ROOT . "templates/_parts/_footer.tpl";
include ROOT . "templates/_parts/_foot.tpl";

?>